...
create index if not exists payment_access_claims_user_idx on public.payment_access_claims (user_id, claimed_at desc);
...
create policy "Users can read own payment claims" on public.payment_access_claims for select to authenticated using (auth.uid() = user_id);
